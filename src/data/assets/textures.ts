...
/** The boundary hands this module a logger (logic/host/window/packs.ts does it at wiring): the DATA file still writes no log itself, it only calls the function it was given. */
export function adoptWarningSink(sink: (line: string) => void): void {
  warnSink = sink;
}
...
/** Startup install: put every pack Rust scanned into memory and RETURN the line describing the chain.
 *
 ...
 *  disk when the module is evaluated; a Tauri pack arrives only through the host's `preloadPacks()`, and
 *  an ESM import is evaluated before the module body, so copying the original caches an EMPTY result
...
function scanPacks(): void {
  ...
  if (!installed && !warnedNotInstalled) {
    // A pure data module cannot log, so the warning is a VALUE: `warnedNotInstalled` is the data and the
    // boundary reads it (logic/host/window/packs.ts::drainWarnings) — one line, once.
    warnedNotInstalled = true;
    warnSink?.("PACKS not installed yet (preloadPacks() has not finished) — falling back to the engine's built-ins for this resolution, not caching it");
  }
}
